...
const TableCell = ({ colItem, data }: TableCellPropTypes) => {
  return colItem?.type === "with-image" ? (
    <td className="px-3 py-3 whitespace-no-wrap border-b border-gray-200">
      <div className="flex items-center">
        <div className="flex-shrink-0 h-10 w-10">
          <img
            className={`ml-6 h-10 w-10 cursor-default ${
              colItem?.imageClassname || "rounded-full"
            }  `}
            src={
              data?.format
        ...
        <div className="ml-8">
          {colItem.rows.map((rowItem, idx) => (
            <div
              className={`flex flex-row items-center justify-center w-full h-full font-light ${
    ...
    <td
      className={`px-4 py-4  border-b border-gray-200  ${aligner(
        colItem?.align,
        "text"
      )}  `}
    >
      {colItem.rows.map((rowItem, idx) => {
        ...
        return rowItem?.text ? (
          <div
            className={`flex flex-row justify-center items-center w-full h-full font-light  ${aligner(
              colItem?.align,
              "text"
            )}`}
            key={idx}
          ...
          <div
            key={idx}
            className={`flex flex-row  ${aligner(colItem?.align, "flex")}`}
          >
            <div
            ...
            >
              <div
                className={`h-1.5 w-1.5 rounded-full ${
                  statusItem?.label === "Active" ? "bg-green-400" : "bg-red-500"
                } `}
              />
              ...
              <span
                ...
                className={`px-2 text-xs whitespace-nowrap  leading-5 font-normal `}
              >
                {statusItem?.label ||
          ...
          <div className={` h-full font-light inline-flex`} key={idx}>
            {rowItem["action"]?.type === "link" ? (
              ...
              <button
                ...
                title={rowItem.action?.title}
                className="w-8 h-8 ml-0.5 mb-0.5 inline-flex rounded-full items-center justify-center  bg-transparent hover:bg-blue-200 focus:outline-none"
              >
                {rowItem.action?.svg
                  ? getSvg(rowItem.action?.svg)
                  : getSvg("none")}
